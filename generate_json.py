...
def parse_txt_library():
    ...
    print("Parsing and strictly cleaning song_library.txt...")
    ...
    for block in blocks:
        ...
        # Comprehensive filtering to drop index artifacts, dividers, and alphabetical headers
        is_junk = (
            set(title) <= {'*', '-', '=', '_', ' ', '—', '.', '·'} or
            len(title) < 2 or
            title.startswith('(') or
    ...
    print(f"SUCCESS: Extracted, cleaned, and sorted {len(songs)} valid songs into {JSON_PATH}.")
    return songs
